...
function getName(name) {
  ...
  if (names.length > 1) {
    return `${names[0].charAt(0).toUpperCase() + names[0].slice(1).toLowerCase()} ${names[names.length - 1].charAt(0).toUpperCase()}.`
  } else {
    return names[0]
  }
}
...
<template>
  <div class="flex justify-end z-50">
    ...
    <div
      ...
      class="absolute right-3 bg-bp_neutral-700 px-4 rounded-xl shadow-lg transition-all ease-in-out duration-300 w-[40%] md:w-[15%] max-h-[35vh] overflow-y-auto custom-scrollbar"
    >
      <div
        class="mb-4 py-4 flex justify-between items-center top-0 bg-bp_neutral-700 z-10 transition-shadow duration-300"
        :class="{ 'shadow-md': isHeaderShadowVisible }"
      >
        <div class="flex items-center gap-2">
          <h4 class="text-sm md:text-base font-medium text-bp_white-100">Pedidos de Amizade</h4>
        </div>
        <button
      ...
      <ul class="flex flex-col gap-2 relative notification-container">
        ...
        >
          <div class="flex flex-wrap items-center gap-4">
            <div>
              <img
                v-if="notification.user.imageUrl"
            ...
            <div class="flex-1">
              <p class="text-sm md:text-base text-bp_white-100">
                {{ getName(notification.user.personName) }}
              </p>
              <div class="flex gap-2 ml-auto">
                <button
                  class="px-4 py-1 rounded-lg text-white bg-bp_green-500 hover:scale-[1.05] duration-300 border-none text-xs md:text-sm"
                  @click="handleUpdateNotificationStatus(notification, 'ACCEPTED')"
                >
                  <v-icon name="fa-thumbs-up" scale="0.9" />
                </button>
                <button
                  class="px-4 py-1 rounded-lg text-white bg-bp_red-200 hover:scale-[1.05] duration-300 border-none text-xs md:text-sm"
